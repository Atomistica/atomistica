...
module direct_coulomb
  ...
  interface potential_and_field
     module procedure direct_coulomb_potential_and_field
  endinterface
...
contains
  ...
  !!<
  subroutine direct_coulomb_potential_and_field(this, p, nl, q, phi, epot, E, wpot, ierror)
    implicit none
...
!    real(DP), intent(inout)                :: wpot(3, 3)
    ...
    do i = 1, p%natloc-1
       ...
       do j = i+1, p%natloc
          ...
          df                 = fac*dr/(abs_dr**3)
          VEC3(tls_vec1, i)  = VEC3(tls_vec1, i) + q(j)*df
          VEC3(tls_vec1, j)  = VEC3(tls_vec1, j) - q_i*df
          tls_sca1(i)        = tls_sca1(i) + fac*q(j)/abs_dr
          tls_sca1(j)        = tls_sca1(j) + fac*q_i/abs_dr
          energy             = energy + fac*q_i*q(j)/abs_dr
          virial             = virial - outer_product(dr, fac*q_i*q(j)*df)
       enddo
    enddo

    call tls_reduce(p%natloc, sca1=phi, vec1=E)
  ...
  endsubroutine direct_coulomb_potential_and_field
